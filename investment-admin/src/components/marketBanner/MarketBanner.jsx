...
const MarketBanner = () => {
  ...
  const { refetch } = useGetAllMarketBannersQuery();

  const handleUpload = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    if (marketPhotos.length > 1) {
      return toast.error("Please upload one image.");
    }
    try {
      const marketBannerPic = await uploadImageToImgBB(marketPhotos[0]);

      const withAttachmentUserData = {
        bannerImage: marketBannerPic,
      };

      await axios.post(`${getBaseUrl()}/market-banner`, withAttachmentUserData);
  ...
  return (
    <div className="col-span-5 xl:col-span-2">
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="border-b border-stroke px-7 py-4 dark:border-strokedark flex justify-between">
          <h3 className="font-medium text-black dark:text-white">
            Add Market Banner
          </h3>
          <Link
            href={"/market-banner-list"}
          ...
          <form onSubmit={handleUpload}>
            ...
            >
              <input
                ...
                id="marketBannerPhoto"
                onChange={(e) => setMarketPhotos(e.target.files)}
                multiple
                required
              />
